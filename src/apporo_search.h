...
namespace apporo {
  namespace search {
    ...
    public:
      ...
      std::vector < std::pair <double, std::string> > rerankAndGetResult(std::vector <std::pair <apporo::strage::sa_index, int> > &id_freq_vec, std::map <apporo::strage::sa_index, apporo::strage::sa_index> &id_index_map, apporo::query::NgramQuery *nq, apporo::strdistance::StringDistance *strdist, int result_num = 0);
    };
  }
}
