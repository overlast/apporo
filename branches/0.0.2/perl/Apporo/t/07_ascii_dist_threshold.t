...
__CONF__
    ...
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.0, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.0 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.1, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.1 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.2, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.2 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.3, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.3 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.399999, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.399999 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.4, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.4 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.428570, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.427580 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.5, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.5 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.599999, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.599999 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.6, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.6 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.7, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.7 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.8, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.8 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.9, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.9 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=0.999999, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    is_deeply(\%hash_arr, \%hash_res, "get the result which is limited by the parameter of dist_threshold:0.999999 from the indexes using '$query' query");
}
...
__CONF__
    ...
    my $is_there_file = 0;
    my $file_path = $conf_path;
    my $file_name = "configure file(ASCII, 2-gram, dist_threshold=1.0, insert dummy character to head and tail of query e.t.c.) of apporo search";
    if( -f $file_path ) { $is_there_file = 1; }
    is($is_there_file, 1, "write $file_name to /tmp");
    my $file_size = -s $file_path;
    isnt($file_size, 0, "$file_name has data entity");
    ...
    my @arr = @{$app->retrieve($query)};
    my @res = ();
    is_deeply(\@arr, \@res, "get the result which is limited by the parameter of dist_threshold:1.0 from the indexes using '$query' query");
}
